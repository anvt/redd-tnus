...
$catid = (!empty($catid) AND !empty($id)) ? $catid : 17;
	...
	if(!empty($id))
	{
		$NEWS_BLOCK = getJobDetail($id);
		$NEWS_BLOCK_OTHER = getJobDetail_Other($id, 3);
	}
	else
	{
		$NEWS_BLOCK = getJob($page,$catid);
		$PAGE_NAVIGATOR = getJobPaging($page,$catid);
	}	
