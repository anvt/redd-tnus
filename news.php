...
$catid = (!empty($catid) AND !empty($id)) ? $catid : 4;
	...
	if(!empty($id))
	{
		$NEWS_BLOCK = getNewsDetail($id);
		$NEWS_BLOCK_OTHER = getNewsDetail_Other($id, 3);
	}
	else
	{
		$NEWS_BLOCK = getNews($page);
		$PAGE_NAVIGATOR = getNewsPaging($page);
	}	
